...
# address2="https://codeload.github.com/GridProtectionAlliance" 
address2="https://github.com" 
myVersion="2.4"
...
if [ $useNightlyBuild -ne 0 ]; then
	...
	#wget ${address}/Products/${service}/Releases/${stableVersion}/POSIX/gsf/Source.zip >> ${log}
    	#wget ${address2}/GridProtectionAlliance/gsf/archive/v${myVersion}.tar.gz -O - | tar -xz >> ${log}
	wget https://github.com/Energetica2030/openPDC-openHistorian-Docker/raw/master/gsf-v2.4.tar.gz  -O - | tar -xz >> ${log}
fi
...
if [ $useNightlyBuild -ne 0 ]; then
    ...
    wget ${address2}/GridProtectionAlliance/${service}/archive/v${myVersion}.tar.gz -O - | tar -xz >> ${log}
    # wget ${address2}/${service}/zip/v${myVersion}.zip >> ${log}
...
echo
#unzip v${myVersion}.zip -d Source >> ${log}
#unzip Source.zip -d Source >> ${log}
#rm -v Source.zip >> ${log}
# rm -v v${myVersion}.zip >> ${log}
cd ..

pwd

# Clear out existing GSF dependencies (prevents possible compile with Windows binaries)
rm -v ${service}/openPDC-${myVersion}/Source/Dependencies/GSF/*.*  >> ${log}
...
wget https://github.com/Energetica2030/openPDC-openHistorian-Docker/raw/master/config-openPDC.tar.gz -O - | tar -xz >> ${log}
...
echo
cp -v -r gsf/Build/Output/Mono/Libraries/* ${service}/openPDC-${myVersion}/Source/Dependencies/GSF  >> ${log}

# Build host application
echo Building ${service} for Mono...
echo
cd ${service}/openPDC-${myVersion}/Source
# cd ${service}/Source
sh buildmono.sh
cd ../../..
...
echo
cp -rv ${service}/openPDC-${myVersion}/Build/Output/Mono/Applications/${service}/* ${destDir} >> ${log}
cp -rv ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}/wwwroot ${destDir} >> ${log}
cp -v ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}/Sample1344.PmuCapture ${destDir}/ >> ${log}
mkdir -v "${destDir}/Database Scripts" >> ${log}
cp -rv ${service}/openPDC-${myVersion}/Source/Data/* "${destDir}/Database Scripts" >> ${log}
cp -rv ${service}/openPDC-${myVersion}/Source/Dependencies/GSF/* ${destDir}/ >> ${log}
cp -v ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}Setup/ConfigCrypter.exe* ${destDir}/ >> ${log}
cp -v ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}Setup/HistorianPlaybackUtility.exe* ${destDir}/ >> ${log}
cp -v ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}Setup/DataMigrationUtility.exe* ${destDir}/ >> ${log}
...
cp -v ${service}/openPDC-${myVersion}/Source/Applications/${service}/${service}Setup/ConfigurationEditor.exe ${destDir}/ >> ${log}
...
if [ $installGEPTester -ne 0 ]; then
	...
	echo
    cd ${service}/Source/openPDC-${myVersion}/Source/Applications/${service}/${service}Setup/
